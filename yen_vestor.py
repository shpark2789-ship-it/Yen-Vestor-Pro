...
def get_usd_status(val):
    if val >= 150: return "🟢 BOJ 개입 임박 (엔화 매수)", "#10b981"
    elif val <= 140: return "🔴 엔화 이미 강세 (추격 금지)", "#ef4444"
    else: return "⚪ 일반적인 흐름", "#94a3b8"

def get_yield_status(val):
    if val >= 4.5: return "🔴 달러 강세 (엔화 약세장)", "#ef4444"
    elif val <= 4.0: return "🟢 금리 인하 (엔화 강세 전환점)", "#10b981"
    else: return "⚪ 중립 (방향성 탐색중)", "#94a3b8"

def get_vix_status(val):
    if val >= 25: return "🔴 패닉장 (엔화 단기 폭등/차익실현)", "#ef4444"
    elif val >= 20: return "🟡 경계장 (안전자산 수요 쏠림)", "#f59e0b"
    else: return "🟢 평온장 (엔화 조용히 매집할 때)", "#10b981"
...
class YenVestorApp:
    ...
    def build_dashboard_tab(self):
        ...
        self.btn_refresh = tk.Button(self.ind_frame, text="[새로고침] 실시간 데이터 및 차트 분석", bg="#3b82f6", fg="white", font=("Arial", 11, "bold"), relief="flat", command=self.refresh_realtime_data, padx=10, pady=5)
        ...
        def create_indicator(parent, title, info_cmd=None):
            ...
            if info_cmd:
                btn_info = tk.Button(top_frame, text="?", bg="#334155", fg="white", font=("Arial", 8, "bold"), relief="flat", command=info_cmd, padx=4, cursor="hand2")
        ...
        tk.Label(chart_header, text="📈 원/엔 환율 종합 기술적 분석 (Price, 이동평균, 볼린저밴드, RSI)", font=("Arial", 12, "bold"), fg="#1e293b").pack(side="left")
        tk.Button(chart_header, text="[?] 기술적 지표란?", bg="#10b981", fg="white", font=("Arial", 9, "bold"), relief="flat", command=self.info_technical, padx=8).pack(side="right")
        ...
        if HAS_MATPLOTLIB:
            ...
            self.ax.text(0.5, 0.5, "우측 상단의 '[새로고침] 실시간 데이터 및 차트 분석' 버튼을 눌러주세요.", 
                         horizontalalignment='center', verticalalignment='center', color='#64748b', fontsize=12, fontweight='bold')
            self.ax.set_xticks([])
            self.ax.set_yticks([])
        ...
        self.btn_analyze = tk.Button(top_ctrl, text="▶ 현재 데이터 기반 AI 알고리즘 가동", font=("Arial", 11, "bold"), bg="#2563eb", fg="white", command=self.start_analysis, relief="flat", padx=20, pady=5)
        ...
        self.lbl_analysis_step = ttk.Label(analysis_frame, text="대기 중...", font=("Arial", 10, "italic"), foreground="#64748b")
        self.lbl_analysis_step.pack()

        self.txt_result = tk.Text(analysis_frame, height=6, state='disabled', bg='#f1f5f9', font=("Arial", 11), relief="flat")
    ...
    def _fetch_data_thread(self):
        try:
            ...
            self.root.after(0, self._apply_fetched_data, krw_jpy_latest, usd_jpy_latest, us_yield_latest, vix_latest, df)
    ...
    def show_analysis_result(self):
        ...
        if self.indicators['rsi'] <= 30:
            score += 20; reasons.append(f"🟣 RSI {self.indicators['rsi']}% (과매도): 시장의 투매가 멈추고 기술적 반등이 강하게 임박했습니다.")
        elif self.indicators['rsi'] >= 70:
            score -= 20; reasons.append(f"🟣 RSI {self.indicators['rsi']}% (과매수): 단기 과열 상태입니다. 곧 조정이 예상됩니다.")
            
        if cur_price <= self.indicators['bb_lower'] * 1.01: # 밴드 하단 근접
            score += 15; reasons.append(f"☁️ 볼린저 밴드 하단 터치: 통계적으로 가격이 위로 튕겨오를 확률이 95% 이상인 저점입니다.")
        elif cur_price >= self.indicators['bb_upper'] * 0.99:
            score -= 15; reasons.append(f"☁️ 볼린저 밴드 상단 터치: 저항선에 부딪혀 곧 가격이 하락할 가능성이 큽니다.")
            
        if cur_price > self.indicators['ma20']:
            score += 5; reasons.append(f"🟡 이동평균선 상회: 20일선 위에 있어 단기 추세가 우상향(상승세)을 타고 있습니다.")
        else:
            score -= 5; reasons.append(f"🟡 이동평균선 하회: 20일선 아래에 있어 단기 추세가 다소 꺾인 상태입니다.")

        # [2] 매크로(거시 경제) 지표 분석
        if self.indicators['usd_jpy'] > 150:
            score += 10; reasons.append("🌎 달러/엔 150엔 돌파: BOJ의 환율 방어 개입 및 금리 인상 압박으로 엔화 가치 상승 기대.")
        
        if self.indicators['us_yield'] > 4.5:
            score -= 10; reasons.append("🌎 미 국채 10년물 강세: 미국 금리가 높아 일본을 이탈하는 자금이 많아 엔화 약세 유지.")
        elif self.indicators['us_yield'] < 4.0:
            score += 15; reasons.append("🌎 미 국채 금리 하락: 미-일 금리차 축소로, 글로벌 자금이 다시 엔화로 몰려들 좋은 환경입니다.")

        if self.indicators['vix'] > 25:
            score += 15; reasons.append("🚨 VIX 공포지수 급등: 글로벌 위기로 인해 안전자산(엔화)의 급격한 수요가 몰리고 있습니다.")

        # 제한 처리
        score = max(0, min(100, int(score)))

        action = 'HOLD (관망)'
        if score >= 80: action = 'STRONG BUY (영혼까지 끌어모아 강력 매수)'
        elif score >= 60: action = 'BUY (분할 매수 권장)'
        elif score <= 20: action = 'STRONG SELL (뒤도 보지 말고 전량 매도)'
        elif score <= 40: action = 'SELL (수익 실현 / 분할 매도)'
    ...
    def update_portfolio_ui(self):
        ...
        if profit_krw > 0: profit_color = "#ef4444" 
        elif profit_krw < 0: profit_color = "#3b82f6" 
        else: profit_color = "black"
            
        self.lbl_profit.config(text=f"평가 손익\n{sign}{profit_krw:,.0f} 원 ({sign}{profit_percent:.2f}%)", foreground=profit_color)
